...
def plot_training_validation( history_1, history_3,history_4, filename=None):
    ...
    plt.plot(history_4['train_loss_4'], label='Model 4 (Train Loss)', linestyle='dashed', color='green')
    ...
    plt.plot(history_4['validation_loss_4'], label='Model 4 (Validation Loss)', linestyle='dashdot', color='green')
    ...
    plt.plot(history_4['train_acc_4'], label='Model 4 (Train Accuracy)', color='green')
    ...
    plt.plot(history_4['validation_acc_4'], label='Model 4 (Validation Accuracy)', linestyle='dotted', color='green')
...
classes_1 = { 5:'Speed limit (80km/h)', 
            6:'End of speed limit (80km/h)' , 
            9:'No passing',  
            12:'Priority road', 
            13:'Yield', 
...
label_mapping = {5: 0, 6: 1, 9: 2, 12: 3, 13: 4, 14: 5, 19: 6, 20: 7, 33: 8, 34: 9, 41: 10}
classes = {label_mapping[class_idx]: label for class_idx, label in classes_1.items()}
...
symbol_indices = list(Symbols.keys())
...
rules = np.array([
    [label_mapping[row[0]], row[1], row[2]] for row in rules])
...
def logical_loss(shape_preds, color_preds, class_preds, rules, Logic, shape_targets, color_targets, class_targets):
    ...
    for i in range(num_rules):
        ...
        class_rule = class_preds_s[:, class_targets]
        shape_rule = shape_preds_s[:, shape_targets]
        color_rule = color_preds_s[:, color_targets]
...
rules_4 = np.array([
    [label_mapping[row[0]], row[1], row[2], row[3]] for row in rules_4])


num_rules_4 = len(rules)
for i in range(num_rules_4):
    class_index = rules_4[i][0]
    shape_index = rules_4[i][1]
    color_index = rules_4[i][2]
    symbol_index = rules_4[i][3]


Logics =['product', 'Godel']
# Logics =['product', 'Godel', 'Lukasiewicz']

def logical_loss_4(shape_preds, color_preds, class_preds, symbol_pred, rules_4, Logic, shape_targets, color_targets, class_targets, symbol_targets):

    num_rules = len(rules)
    product = []
    satisfaction = []
    for i in range(num_rules):
        class_index, shape_index, color_index, symbol_index = rules[i]

        class_preds_s  = F.softmax(class_preds, dim=1)
        shape_preds_s  = F.softmax(shape_preds, dim=1)
        color_preds_s  = F.softmax(color_preds, dim=1)
        symbol_preds_s = F.softmax(symbol_preds, dim=1)

        class_rule = class_preds_s[:, class_targets]
        shape_rule = shape_preds_s[:, shape_targets]
        color_rule = color_preds_s[:, color_targets]
        symbol_rule = symbol_preds_s[:, symbol_targets]


        if Logic == 'product':
            product =shape_rule * color_rule * class_rule*symbol_rule
            satisfaction.append(torch.mean(product).item())

        elif Logic == 'Godel':
            product = torch.minimum(torch.minimum(torch.minimum(shape_rule, color_rule), class_rule), symbol_rule)
            satisfaction.append(torch.mean(torch.tensor(product)).item())

        elif  Logic =='Lukasiewicz':
            product = (shape_rule + color_rule + class_rule + symbol_rule - 3)
            satisfaction.append(torch.mean(product).item())       
...
class CustomModel_1(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('layer_flat', layer_flat.size(1))
...
class CustomModel_4(nn.Module):
    def __init__(self, num_shape_classes, num_color_classes, num_class_classes, num_symbol_classes):

        super(CustomModel_4, self).__init__()
        ...
        self.symbol_classifier = nn.Linear(fc_size2, num_symbol_classes)
    ...
    def forward(self, x):
        ...
        symbol_output = self.symbol_classifier(x)

        return shape_output, color_output, class_output, symbol_output
    ...
    for itr in range (itrs):
        for alpha in alphas:
            if alpha ==0:
                ...
                model_4  = CustomModel_4(num_shape_classes= shape_num, num_color_classes=colour_num, num_class_classes=class_num, num_symbol_classes=symbol_num)  # Include num_class_classes
                ...
                optimizer_4      = optim.Adam(model_4.parameters(), lr=0.001, weight_decay=1e-5)
                ...
                for epoch in range(num_epochs):
                    ...
                    for images, _, _, _,  class_targets in train_dataloader:
                        class_targets = torch.tensor([label_mapping[label.item()] for label in class_targets], dtype=torch.int64)

                        
                        optimizer_1.zero_grad()
                        class_preds = model_1(images)
                        # print("Unique target values:", torch.unique(class_targets))
                        # print("class_preds:", class_num)
                    ...
                    for images, shape_targets, color_targets, symbol_targets, class_targets in train_dataloader:
                        ...
                        shape_preds, color_preds, class_preds, symbol_preds = model_4(images)

                        shape_loss = shape_criterion(shape_preds, shape_targets)
                        color_loss = color_criterion(color_preds, color_targets)
                        class_loss = class_criterion(class_preds, class_targets)  # Include class loss
                        symbol_loss = symbol_criterion(symbol_preds, symbol_targets)
                        # print('class_loss', class_loss)    
                        # Calculate the total loss by summing shape, color, symbol, and class losses
                        train_loss = shape_loss + color_loss + class_loss + symbol_loss
                        losses.append(train_loss.item())
                        total += class_targets.size(0)
                        # Backpropagation and optimization
                        train_loss.backward()
                    ...
                    scheduler_4.step()
                    train_acc_4  = 100 * correct / total
                    train_loss_4 = np.mean(losses)  
                    print(f'model_4_Alpha [{alpha}] - Epoch [{epoch + 1}/{num_epochs}] -   train_acc_4: {train_acc_4} -   train_loss_4: {train_loss_4}')
                    ...
                    with torch.no_grad():
                        for images, shape_targets, color_targets, symbol_targets, class_targets in validation_dataloader:
                            ...
                            loss_val = class_criterion(class_preds, class_targets)
                            losses.append(loss_val.item())
                    validation_acc_4  = 100 * correct / total
                    validation_loss_4 =  np.mean(losses) 
                    print(f'model_4_ Epoch [{epoch + 1}/{num_epochs}] -   validation_acc_4: {validation_acc_4}-   validation_loss_4: {validation_loss_4}')
    
    
                    history_4['train_acc_4'].append(train_acc_4)
                    history_4['train_loss_4'].append(train_loss_4)
                    history_4['validation_acc_4'].append(validation_acc_4)
                    history_4['validation_loss_4'].append(validation_loss_4)
    
    
                    if validation_acc_4 > best_accuracy_4:
                        best_accuracy_4 = validation_acc_4             
                        b_epoch_4    = epoch
    
                        best_model_state_dict_4 = copy.deepcopy(model_4.state_dict())
        

                
                print(f'b_epoch_1 [{b_epoch_1}]-best_accuracy_1: {best_accuracy_1} - b_epoch_3 [{b_epoch_3}]-  best_accuracy_3: {best_accuracy_3} - b_epoch_4 [{b_epoch_4}]-  best_accuracy_4: {best_accuracy_4}')
                ...
                model_4_path     = os.path.join(save_dir, f'{subfolder}_alpha{alpha}_NoLogic_best_model_4_itr_{itr}.pth')

                torch.save(best_model_state_dict_1, model_1_path)
                torch.save(best_model_state_dict_3, model_3_path)
                torch.save(best_model_state_dict_4, model_4_path)
                ...
                print('file_name=', f'{subfolder_name}_{alpha}_{itr}.png')
                plot_training_validation( history_1,history_3,history_4, filename=f'{subfolder_name}_{alpha}_{itr}.png')
                ...
                for Logic in Logics:
                    ...
                    for epoch in range(num_epochs):
                        ...
                        for images, shape_targets, color_targets, symbol_targets, class_targets in train_dataloader:
                            ...
                            logic_loss = logical_loss(shape_preds, color_preds, class_preds, rules, Logic, shape_targets, color_targets, class_targets)
                        ...
                        print(f'b_epoch_4 [{b_epoch_4}]-best_accuracy_4: {best_accuracy_4} - b_epoch_3 [{b_epoch_3}]-  best_accuracy_3: {best_accuracy_3}')
                    ...
                    model_4_path     = os.path.join(save_dir, f'{subfolder}_alpha{alpha}_Logic_{Logic}_best_model_4_itr_{itr}.pth')
                    torch.save(best_model_state_dict_4, model_4_path)
                    ...
                    history_file_path_3 = os.path.join(history_folder, f'history_3_{subfolder}_alpha{alpha}_Logic{Logic}_itr_{itr}.pkl')
                    with open(history_file_path_3, 'wb') as history_file:
                        pickle.dump(history_3, history_file)
                    
                    history_file_path_4 = os.path.join(history_folder, f'history_4_{subfolder}_alpha{alpha}_Logic{Logic}_itr_{itr}.pkl')
                    with open(history_file_path_4, 'wb') as history_file:
                        pickle.dump(history_3, history_file)
                        
                        plot_training_validation( history_1,history_3,history_4, filename=f'{subfolder_name}_{Logic}_{alpha}_{itr}.png')
